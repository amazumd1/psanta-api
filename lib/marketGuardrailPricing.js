...
const doc = await MarketRateProfile.findOne({
  zip: cleanZip,
  service: normalizedService,
  $or: [
    { status: 'verified' },
    { status: { $exists: false } }, // old rows remain usable
  ],
}).lean();
...
async function computeMarketGuardrailQuote(input = {}, cfgQuote = null) {
  ...
  // MarketRateProfile is already ZIP + service specific.
  // Example: 33140 + turnover_clean already means STR turnover market median.
  // Do NOT multiply price by serviceFactor again, or turnover/deep/move-out get double-counted.
  const rawServiceFactor = serviceFactor(service);
  const factorServiceForPrice = 1;
  const factorServiceForMinutes = rawServiceFactor;
  ...
  const aiMarketEstimate =
    baseMarket * factorSize * factorServiceForPrice * factorCondition * factorUrgency + addonsTotal;
  ...
  const minutes =
    cfgQuote?.breakdown?.minutes ||
    Math.round(75 * factorSize * factorServiceForMinutes * factorCondition);
  ...
  return {
    ...
    breakdown: {
      ...
      serviceFactor: factorServiceForPrice,
      minutesServiceFactor: factorServiceForMinutes,
      serviceFactorSuppressedForPrice: rawServiceFactor !== 1,
      marketRateBasis: 'zip_service_profile',
