...
const Job = require(path.join(__dirname, "..", "src", "models", "Job"));
const Counter = require(path.join(__dirname, "..", "src", "models", "Counter"));
...
(async () => {
  ...
  console.log(
    `Jobs without jobId: ${totalToFix}, next will start from ${Math.max(
      currentMax + 1,
      START_AT
    )}`
  );
